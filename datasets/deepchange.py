...
class DeepChange(object):
    ...
    def __init__(self, root='data', **kwargs):
        ...
        train, num_train_pids, num_train_clothes, num_train_cameras = self._process_dir(self.train_dir, train_names, clothes2label, pid2label=pid2label)
        # pid2label, clothes2label, pid2clothes = self.get_pid2label_and_clothes2label(train_names+val_query_names+val_gallery_names)
        # train, num_train_pids, num_train_clothes, num_train_cameras = self._process_train_dir([self.train_dir, self.val_query_dir, self.val_gallery_dir],
        #                                                                                       [train_names, val_query_names, val_gallery_names],
        #                                                                                       clothes2label,
        #                                                                                       pid2label=pid2label)
        ...
        self.num_train_cams = num_train_cameras
    ...
    def _process_train_dir(self, home_dirs, img_names, clothes2label, pid2label=None):
        dataset = []
        pid_container = set()
        clothes_container = set()
        cams_container = set()
        for home_dir, img_names_ in zip(home_dirs, img_names):
            ...
            num_pids = len(pid_container)
            num_clothes = len(clothes_container)
            num_camera = len(cams_container)

        return dataset, num_pids, num_clothes, num_camera
