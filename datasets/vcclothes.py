...
class VCClothes(object):
    ...
    def __init__(self, root='data', mode='all', **kwargs):
        ...
        self.parsing_dir = osp.join(self.dataset_dir, 'parsing')
        # 'all' for all cameras; 'sc' for cam2&3; 'cc' for cam3&4
        self.mode = mode
        self._check_before_run()

        train, num_train_pids, num_train_imgs, num_train_clothes, pid2clothes, num_train_cams = self._process_dir_train()
        train_parsing = self._process_dir_parsing()
        ...
        self.train_parsing = train_parsing[0]
    ...
    def _process_dir_parsing(self):
        img_paths = glob.glob(osp.join(self.parsing_dir, '*.png'))
        img_paths.sort()
